...
本仓库是一个可复用的非 Roblox 游戏 Wiki 模板。新项目允许根据游戏调整主题、字体、图片、间距和布局表现，但必须保留现有页面框架、路由、MDX、SEO、多语言、法律页、响应式行为和广告位置。
...
- Adsterra KEY 写入 `.env.local` 的 `NEXT_PUBLIC_AD_*`，运行 `pnpm ads:sync`（`dev`/`build` 会自动执行）。不要提交 `.env.local`。详见 `public/ads/README.md`。
- 默认保证本地构建和验证通过；用户明确要求时，可以配置并执行远程部署。
